...
bool message_get_fields(MessageFieldsData* fields_data, void* message, uint32_t size) {
	...
	uint8_t* body = utils_poniter_calc<uint8_t*>(message, sizeof(MessageHeader));
	//uint32_t body_size = size - sizeof(MessageHeader);
	
	uint8_t* data = utils_poniter_calc<uint8_t*>(body, 4 * fieldc);
		...
		if (NULL != offsets) {
			...
			for (uint32_t i = 0; i < fieldc; ++i) {
				...
				if (field_size) {
					...
					memcpy(fields_data->fields_data[i], utils_poniter_calc<uint8_t*>(data, cur_offset), field_size);
				}
			}
			rv = true;
		}
...
bool message_build(uint16_t type, MessageFieldsData* data, void* buffer, uint32_t buffer_size) {
	...
	if (message_size <= buffer_size) {
		...
		for (uint32_t i = 0; i < data->fieldc; ++i) {
			...
			*utils_poniter_calc<uint32_t*>(buffer, offset) = field_offset;
			offset += 4;
		}
		...
		for (uint32_t i = 0; i < data->fieldc; ++i) {
			...
			if (field_size) {
				memcpy(utils_poniter_calc<uint8_t*>(buffer, offset), data->fields_data[i], field_size);
				offset += field_size;
			}
		}
...
bool message_field_add(MessageFieldsData* data, uint8_t* buffer, uint32_t buffer_size) {
	...
	memcpy(data->fields_data[i], buffer, buffer_size);
